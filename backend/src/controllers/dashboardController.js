...
const stats = async (req, res) => {
  const [totalProductos, movimientosHoy, alertas, valorInventario, movimientosPorDia, porCategoria] = await Promise.all([
    query('SELECT COUNT(*)::int AS total FROM productos WHERE activo = true'),
    query(`SELECT COUNT(*)::int AS total FROM movimientos WHERE created_at::date = CURRENT_DATE`),
    query(`
    ...
    `),
    query(`
      SELECT COALESCE(SUM(s.cantidad * p.precio_unitario), 0)::numeric(14,2) AS total
      FROM stock s JOIN productos p ON p.id = s.producto_id
    `),
    query(`
      SELECT to_char(d.dia, 'YYYY-MM-DD') AS fecha,
        COALESCE(SUM(CASE WHEN m.tipo IN ('entrada','devolucion') THEN m.cantidad ELSE 0 END), 0)::int AS entradas,
        COALESCE(SUM(CASE WHEN m.tipo = 'salida' THEN m.cantidad ELSE 0 END), 0)::int AS salidas
    ...
    `),
    query(`
      SELECT c.nombre, COUNT(p.id)::int AS total
  ...
  res.json({
    ...
    alertasStock: alertas.rows[0].total,
